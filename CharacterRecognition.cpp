		...
		cv::Rect &charRect)
{
	...
	Point rect_center(
			boundRect[boundRect.size() - 1].tl()
					+ Point(boundRect[boundRect.size() - 1].width / 2,
...
#pragma omp parallel for
	...
	{
		int mindis = thresImgv[0].cols * thresImgv[0].cols
				+ thresImgv[0].rows * thresImgv[0].rows;
		int maxdis = 1;
		int maxidx = 0;
		int minidx = 0;
		for (size_t j = 0; j < 4; j++)
		{
			int a = PolyCanditates[i][j].x * PolyCanditates[i][j].x
					+ PolyCanditates[i][j].y * PolyCanditates[i][j].y;
		...
		if (abs(PolyCanditates[i][0].x - PolyCanditates[i][1].x)
				> abs(PolyCanditates[i][0].y - PolyCanditates[i][1].y))
		{
...
#pragma omp parallel for
	for (unsigned int i = 0; i < PolyCanditates.size(); i++)
	{
		...
		for (unsigned int j = i + 1; j < PolyCanditates.size(); j++)
		{
			...
			{
				vdist[c] = sqrt(
						(PolyCanditates[i][c].x - PolyCanditates[j][c].x)
								* (PolyCanditates[i][c].x
										- PolyCanditates[j][c].x)
								+ (PolyCanditates[i][c].y
										- PolyCanditates[j][c].y)
										* (PolyCanditates[i][c].y
												- PolyCanditates[j][c].y));
			}

			// if distance is too small
			if (vdist[0] < 10 && vdist[1] < 10 && vdist[2] < 10
					&& vdist[3] < 10)
			{
	...
	{
		if (perimeter(PolyCanditates[TooNearCandidates[i].first])
				> perimeter(PolyCanditates[TooNearCandidates[i].second]))
	...
	for (size_t i = 0; i < outPolyCanditates.size(); i++)
	{
		...
		{
			if (outPolyCanditates[i][c].x < borderDistThresX
					|| outPolyCanditates[i][c].y < borderDistThresY
					|| outPolyCanditates[i][c].x
							> thresImgv[0].cols - borderDistThresX
					|| outPolyCanditates[i][c].y
							> thresImgv[0].rows - borderDistThresY)
			{
				toRemove1[i] = true;
			}
		}
		...
		std::vector<CharacterImg> &charRectImg, cv::Size size_)
{
	...
	for (size_t i = 0; i < polys.size(); i++)
	{
		...
		int diff_ave = (s1[0] - s2[0])
				/ (thresimg.cols * thresimg.rows - r.area());
